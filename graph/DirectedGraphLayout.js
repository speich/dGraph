...
export class DirectedGraphLayout {
	 ...
	 */
	addEdges(nodeList, adjList) {
		let i, j, len, lenJ, target, source,
	 ...
	 */
	countCrossings(layer, direction) {
		let propName = direction === 'up' ? 'trgNodes': 'srcNodes',
	 ...
	 * @return {Boolean}
	 */
	orderLayer(layer, direction) {
		let nodeOrderRestore = this.nodes[layer].slice(0),
	 ...
	 */
	getMedian(node, type) {
		let weights = [], i, len, middle, nodes;
	 ...
	 *
	 */
	align(layer, direction) {
		// move node according to it's upper/lower median
		let numDec = this.getDecimalPlaces(this.incVirt, '.'),
	 ...
	 * @return {Number}
	 */
	getDecimalPlaces(x, decSeparator) {
		let str = x.toString();
